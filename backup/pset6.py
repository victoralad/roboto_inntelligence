...
for i in range(I):
    ...
    mseLoss = 0.5 * np.matmul((Y - y_hat), (Y - y_hat).transpose())
    L.append(mseLoss[0][0])                                  			 # Compute the MSE loss at this iteration for the entire training data set (not just the samples chosen)
    ...
    for j in range(len(nn)):
        nn[j][0] = nn[j][0] - alpha*g[j][0]                      # Update the neural network weights
